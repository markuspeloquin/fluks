 ...
 * \param[in] hash_type	The type of hash to use.
 * \param[out] out	The output buffer, which is assumed to be of size
 *	<code>(sz * stripes)</code> bytes.
 */
void	af_split(const uint8_t *in, size_t sz, size_t stripes, enum hash_type,
	    uint8_t *out);
 ...
 */
void	af_merge(const uint8_t *in, size_t sz, size_t stripes, enum hash_type,
	    uint8_t *out);
